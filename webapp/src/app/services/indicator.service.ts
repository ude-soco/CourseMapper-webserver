...
export class IndicatorService {
  ...
  fetchIndicators():  Observable<Indicator[]> {
    ...
    return this.http.get<Indicator[]>(`${this.API_URL}/courses/${courseId}/indicators`).pipe(tap(indicators => {      
  ...
  addNewIndicator(indicator) : Observable<Indicator> {
    const courseId = this.courseService.getSelectedCourse()._id;
    return this.http.post<any>(`${this.API_URL}/courses/${courseId}/indicator`, indicator)
    .pipe(
  ...
  deleteIndicator(indicatorTD: Indicator, courseId){
    return this.http.delete<any>(`${this.API_URL}/courses/${courseId}/indicator/${indicatorTD._id}`)
    .pipe(
      ...
      tap( res => {
        if ( !('errorMsg' in res) ){
          this.indicators = this.indicators.filter(indicator => indicator._id.toString() !== indicatorTD._id.toString());
          this.onUpdateIndicators$.next(this.indicators);
        }
      }));
  }
