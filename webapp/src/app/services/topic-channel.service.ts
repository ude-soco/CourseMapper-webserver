...
export class TopicChannelService {
  ...
  renameTopic(topicTD: Topic, body:any){
    return this.http.put<any>(`${this.API_URL}/courses/${topicTD.courseId}/topics/${topicTD._id}`,body)
    .pipe(
  ...
  getAllTagsForCurrentChannel(channel: Channel): Observable<Tag[]>{
    return this.http.get<Tag[]>(`${environment.API_URL}/courses/${channel.courseId}/channels/${channel._id}/tags`);
  }
}
