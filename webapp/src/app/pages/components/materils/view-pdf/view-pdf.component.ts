...
import { Position, Rectangle, RectangleObject } from 'src/assets/Data/AnnotationForms';
...
export class ViewPdfComponent implements OnInit {
  ...
  constructor( private pdfViewService: PdfviewService) {this.getDocUrl() }
  ...
  public handlePdfLoaded(pdf: any): void {
    ...
    this.pdfComponent.pdfViewer.eventBus.on('updatefindmatchescount', (data: any) => {
     this.matchesFound = data.matchesCount.total
     console.log('total matches found: ', data.matchesCount.total);
     });

    this.pdfComponent.pdfViewer.eventBus.on('updatefindcontrolstate', (data: any) => {
      if (data.state === 0) {
