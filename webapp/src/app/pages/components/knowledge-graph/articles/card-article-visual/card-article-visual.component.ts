...
@Component({
  selector: 'app-card-article-visual',
  templateUrl: './card-article-visual.component.html',
  styleUrls: ['./card-article-visual.component.css'],
})
export class CardArticleComponentVisual {
...
expandedKeyphrases.forEach(({ text: kp, dnu, original }) => {
  ...
  if (kp.length <= 3) {
    // For very short keyphrases like "a", "an", "x", use word boundaries
    regex = new RegExp(`\\b${this.escapeRegex(kp)}\\b`, "gi");
  } else {
    // For longer keyphrases, match literally anywhere (Unicode-safe)
