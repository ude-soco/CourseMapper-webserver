...
exports.getAllCourses = function (req, res) {
  Course.find({}).populate("topics", "-__v").exec(function (err, courses) {
    if (err) {
      res.status(500).send({
        message: err
      });
...
exports.getCourse = function (req, res) {
  ...
  }).populate("topics channels", "-__v").exec(function (err, foundCourse) {
    if (err) {
      res.status(500).send({
        message: err
      });
...
exports.newCourse = function (req, res) {
  Course.findOne({
    ...
    course.save(function (err, course) {
      ...
      res.send({
        id: course._id,
        success: "New course '".concat(course.name, "' added!")
      });
    });
  });
};
...
exports.deleteCourse = function (req, res) {
  Course.findByIdAndRemove({
    ...
    res.send({
      success: "Course \"".concat(foundCourse.name, "\" successfully deleted!")
    });
  });
};