  ...
  try {
    ...
    if (response && response.status === 200) {
      ...
      if (typeof response.data === 'string' && response.data.includes('<!doctype')) {
        console.warn('WARNING: LRS returned HTML instead of JSON. This indicates LRS misconfiguration.');
        console.warn('Using statement IDs from request as workaround.');
      ...
      // Unexpected format
      console.warn('WARNING: LRS returned unexpected format:', typeof response.data);
      return [];
    }
  } catch (error) {
    // in case there is a problem in connection like the LRS is down. status code 443
