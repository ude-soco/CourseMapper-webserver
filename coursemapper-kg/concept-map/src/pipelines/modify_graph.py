...
class ModifyGraphPipeline:
    ...
    def add_concept(self, material_id: str, concept_name: str, slides: list[int]| None, isNew:bool, isEditing:bool, lastEdited:bool) :
        ...
        # Add concept node
        concept_node = Node(f'{material_id}_concept_{str(abs(hash(str(wikipedia_page_embedding))))}', concept_name, '', 'main_concept', (concept_material_weight + 1) / 2, f'https://en.wikipedia.org/wiki/{concept_name}', wikipedia_page.summary, False, isNew , isEditing, lastEdited, wikipedia_page_embedding, []   )
