...
Run command:
    pipenv run python -m app.services.course_materials.Mooc_Recommendation.recommendation_list.mooc_recommendation_list
"""
    ...
    def get_user_embedding(self, user_id):
        with self.coursemapper_connection.get_session() as session:
            record = session.run("""
                ...
                WHERE u.learner_model_embedding_MOOC IS NOT NULL
                RETURN u.learner_model_embedding_MOOC AS embedding
            """, uid=user_id).single()

        if record is None:
            print(f"No learner_model_embedding_MOOC found for uid: {user_id}")
            return None
