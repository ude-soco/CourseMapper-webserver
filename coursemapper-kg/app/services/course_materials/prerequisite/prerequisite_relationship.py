...
class PrerequisiteRelationship:
    def __init__(self, clean_data = pd.DataFrame(), related_relationships = pd.DataFrame()):
        ...
        self.related_relationships = related_relationships
        ...
        for i in range(len(main_concepts)):
            ...
            for j in range(i+1, len(main_concepts)):
                if main_concepts[j] == None:
                    continue
                self.prerequisite_criteria(main_concepts[i],main_concepts[j],0.3)
    ...
    def unweighted_voting(self,c1,c2,c1_to_c2,c2_to_c1,theta, weighted = False):
        if weighted:
        ...
        if delta >= theta:
            preq_rel = {"prerequisite_concept":c1, "concept":c2, "weight": weighted, "score" : abs(delta)}
            if not weighted:
                self.prereq_unweighted.append(preq_rel)     
            else:
                self.prereq_weighted.append(preq_rel)
        if delta <= -theta:
            preq_rel = {"prerequisite_concept":c2, "concept":c1, "weight": weighted,  "score" : abs(delta)}
            if not weighted:
                self.prereq_unweighted.append(preq_rel)
            else:
                self.prereq_weighted.append(preq_rel)
    ...
    def weighted_voting(self,c1,c2,c1_to_c2,c2_to_c1,theta):
        ...
        c1_to_c2 = np.multiply(c1_to_c2,weights)
        c2_to_c1 = np.multiply(c2_to_c1,weights)
        self.unweighted_voting(c1,c2,c1_to_c2,c2_to_c1,theta,weighted=True)
    ...
    def prerequisite_criteria(self,c1,c2,theta):
        ...
        c1_to_c2[4], c2_to_c1[4] = self.refD(c1,c2,0.2) # RefD 
        ...
        c1_to_c2[8], c2_to_c1[8] = self.berttopic(c1,c2) # berttopic
        c1_to_c2[9], c2_to_c1[9] = self.coursemapper_channel(c1,c2) #coursemapper structure
        self.unweighted_voting(c1,c2,c1_to_c2,c2_to_c1,theta)
    ...
    def refD(self,c1,c2,theta):
        #degree at which c1 is mentioned in c2's related concept's abstract
        c1_rel_concepts = self.related_relationships["name"].loc[self.related_relationships["related_to"] == c1]
        c2_rel_concepts = self.related_relationships["name"].loc[self.related_relationships["related_to"] == c2]
        all_rel_concepts = c1_rel_concepts + c2_rel_concepts
        all_rel_concepts = list(dict.fromkeys(all_rel_concepts))
        eq1 = 0
        eq2 = 0
        eq3 = 0
        eq4 = 0

        for concept in all_rel_concepts:
            rel_concepts = self.related_relationships["name"].loc[self.related_relationships["related_to"] == concept]
            if (c1 in rel_concepts):
                eq2 +=1
                if (concept in c2_rel_concepts):
                    eq1 +=1
            if (c2 in rel_concepts):
                eq4 +=1
                if (concept in c1_rel_concepts):
                    eq3 +=1
        try:
            ref_d = (eq1/eq2) - (eq3/eq4)
        except:
            ref_d = 0
        if ref_d > theta:
            return 1,0
