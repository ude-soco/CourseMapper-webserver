...
class DBpediaSpotlight:
    ...
    def annotate(self, materialId, text, whole_text, keyphrases=[]):
        ...
        try:
            ...
            if whole_text:
                ...
                if keyphrases:
                    texts=list(map(lambda x: x[0], keyphrases))
                    # texts.append(". ".join(
    ...
    def build_path1(self,
        ...
        print('Length of lm: '+ str(len(lm_text)))
        ...
        try:
            if with_doc_sim:
                ...
                for node in concepts:
                    # if node["type"] == "annotation" or node["type"] == "property":
                    ...
                    #         node, doc_embeddings)
                    #     node["weight"] = score
                # threshold = int(len(concepts) * 0.5)
                # concepts = sorted(concepts, key=lambda x: x["weight"], reverse=True)[0:threshold]
                concepts = sorted(concepts, key=lambda x: x["weight"], reverse=True)
                concepts_list=[]
    ...
    def build_path(self,
        ...
        try:
            if with_doc_sim:
                ...
                for node in concepts:
                    # if node["type"] == "annotation" or node["type"] == "property":
                    if node["name"] not in concepts_list:# if concept extracted from slide level does not appear in material level
                        continue
                    for data in datas:
                        if node["name"] == data["name"]:
                            node["id"]=data["id"]
                            score = self._get_semantic_similarity_score(doc_embeddings, data["initial_embedding"])
                            node["slide_weight"] = score
                            slide_concepts.append(node)
                            # slide_list.append(node["name"])
                # print(slide_concepts)
                # print(slide_list)
                    # if node["type"] == "category":
                    #     score = self._calculate_similiraty_weight(
                    #         node, doc_embeddings)
                    #     node["weight"] = score
                threshold = 5
                # slide_concepts = sorted(slide_concepts, key=lambda x: x["slide_weight"], reverse=True)[0:threshold]
                slide_concepts = sorted(slide_concepts, key=lambda x: x["slide_weight"], reverse=True)
                for node in slide_concepts:
                    slide_list.append(node["name"])
            ...
            return slide_concepts,slide_list
        except Exception as e:
            logger.error("Failure due to - %s" % e)
    ...
    def _get_related_concepts_and_categories(self, annotations, with_category, with_property,text=" ",other_concepts=[]):
        ...
        try:
            ...
            try:
                if text != " ":
                    # Check relationship between related concepted and other main concept
                    for other_concept in other_concepts:
                        try:
                            if with_category:
                                # logger.info("--------------- with category")
                                self.get_categories(annotation=other_concept, all_annotations=all_annotations)
                            if with_property:
                                # logger.info("--------------- with property")
