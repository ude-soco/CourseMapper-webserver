...
class DBpediaSpotlight:
    ...
    def annotate(self, materialId, text, whole_text, keyphrases=[]):
        ...
        try:
            ...
            if whole_text:
                ...
                if keyphrases:
                    texts.append(". ".join(list(map(lambda x: x[0], keyphrases))))
            for _text in texts:
                ...
                r = requests.get(
                    url=self.url, headers=headers, params=params, verify=False
                ).json()  # activate when DBpedia certificate expires
            ...
            if with_doc_sim:
                ...
                for node in concepts:
                    if node["type"] == "annotation":
                        ...
                        slideScore = self._get_semantic_similarity_score(
                            doc_embeddings, node["initial_embedding"]
                        )
                        score = self._get_semantic_similarity_score(
                            lm_embeddings, node["initial_embedding"]
                        )
                    ...
                    node["id"] = str(
                        abs(hash(str(ann_embeddings)))
                    )  # set id to be initial embeddings as it is the only unique value

                concepts = sorted(
                    concepts,
                    key=lambda x: x["slide_weight"] + x["weight"],
                    reverse=True,
                )
    ...
    def _get_path(self, node_a, node_b):
        ...
        try:
            query = """
                ASK { <%s> dbo:wikiPageWikiLink{,1}/dcterms:subject{,1}/skos:broader{,1} <%s> }
            """ % (
                node_a["uri"],
                node_b["uri"],
            )
                    ...
                    node_a["to"].append(
                        {
                            ...
                            "name": node_b["name"],
                            "type": node_b["type"],
                            "rel_type": self._relationship_type(node_a, node_b),
                            "path": True,
                            "weight": edge_weight,
                        }
                    )
    ...
    def expand(
        self, whole_text, materialId, text, keyphrases, with_category, with_property
    ):
        """ """
        all_annotations = []
        try:
                ...
                try:
                    ...
                    if with_property:
                        ...
                        self._get_related_concepts(
                            annotation=annotation, all_annotations=all_annotations
                        )
                except Exception as e:
                    logger.error("Failed expanding '%s' - %s" % annotation["uri"], e)
        except Exception as e:
    ...
    def _get_categories(self, annotation, all_annotations, doc_embeddings):
        ...
        try:
            ...
            logger.debug("Found %s categories " % len(results["results"]["bindings"]))
            for result in results["results"]["bindings"]:
                ...
                if not self._exists_rel(node, annotation):
                    ...
                    node["weight"] = self._get_semantic_similarity_score(
                        doc_embeddings, node["initial_embedding"]
                    )
                    node["to"].append(
                        {
            ...
            concepts = sorted(
                concepts, key=lambda x: x["weight"] + x["concept_weight"], reverse=True
            )[0:threshold]
            ...
            logger.error(
                "Failed to get categories for %s - %s" % (annotation["uri"], e)
            )
    ...
    def _get_related_concepts(self, annotation, all_annotations, doc_embeddings):
        ...
        try:
            ...
            logger.debug(
                "Found %s related concepts" % len(results["results"]["bindings"])
            )
            ...
            for result in results["results"]["bindings"]:
                ...
                try:
                    if not self._exists_rel(annotation, node):
                        ...
                        node["id"] = str(
                            abs(hash(str(node["initial_embedding"])))
                        )  # set id to be initial embeddings as it is the only unique value
                        edge_weight = self._get_semantic_similarity_score(
                            annotation["initial_embedding"], node["initial_embedding"]
                        )
            ...
            logger.error(
                "Failed to get properties for %s - %s" % (annotation["uri"], e)
            )

    def _get_related_concepts_and_categories(
        self, annotations, with_category, with_property, text=" ", other_concepts=[]
    ):
        """ """
        all_annotations = []
        doc_embeddings = self._get_embeddings(text)
                ...
                try:
                    if with_category:
                        ...
                        self._get_categories(
                            annotation, all_annotations, doc_embeddings
                        )
                    if with_property:
                        logger.info("--------------- with property")
                        self._get_related_concepts(
        ...
        try:
            for annotation in annotations:
                ...
                try:
                    ...
                    logger.error("Failed checking '%s' - %s" % annotation["uri"], e)
        except Exception as e:
            logger.error("Failure in expansion - %s", e)
    ...
    def check_category_relationship(self, annotation, all_annotations):
        ...
        try:
            ...
            categories_result = list(
                set(categories) - set(annotation["category"])
            ) + list(set(annotation["category"]) - set(categories))
    ...
    def check_related_relationship(self, annotation, all_annotations):
        ...
        try:
            ...
            related_result = list(set(related) - set(annotation["related"])) + list(
                set(annotation["related"]) - set(related)
            )
        ...
        try:
            ...
            for result in results["results"]["bindings"]:
                ...
                for concept in all_annotations:
                    if node["id"] == concept["id"]:
                        edge_weight = self._get_semantic_similarity_score(
                            annotation["initial_embedding"],
                            concept["initial_embedding"],
                        )
                        concept["to"].append(
                            {
                                "id": annotation["id"],
    ...
    def _assign_category_weight(self, cat_node, text, doc_embeddings, concepts):
        """ """
        ...
        try:
            score = (
                self._get_node_weight_cf(cat_node, text, concepts)
                * (1 / math.log(abs(int(SP))))
                * (1 / math.log(abs(int(SC))))
            )
            return score
    ...
    def _assign_property_weight(self, prop_node, text, concepts):
        ...
        try:
            ...
            score = self._get_node_weight_cf(prop_node, text, concepts) * (
                1 / math.log(abs(int(P)))
            )
            return score
        except (ZeroDivisionError, ValueError) as e:
            return 0
    ...
    def _get_pages_of_category(self, category):
        ...
        try:
            ...
            logger.error(
                "Failed to count pages of category %s - %s" % (category["name"], e)
            )
            return 0

    def _get_subcategories_of_category(self, category):
        """ """
        logger.info("Counting subcategories of category: %s" % category["name"])
        ...
        try:
            ...
            logger.error(
                "Failed to count subcategories of category %s - %s"
                % (category["name"], e)
            )
            return 0
    ...
    def _get_node_weight_cf(self, node, text, concepts):
        ...
        try:
            ...
            for _node in concepts:
                _ns = _node["to"]
                if any(
                    node["name"].split(":")[-1] == _n["name"].split(":")[-1]
                    for _n in _ns
                ):
                    countInExpansion += 1
            ...
            logger.error("Failed to get node weight for %s - %s" % (node["name"], e))
            return 0
    ...
    def _get_edge_weight(self, source, target, beta=0.5, max_length=2):
        ...
        for l in range(1, max_length + 1):
            sum += math.pow(beta, l) * float(self._count_path_length(source, target, l))
        score = 1 - (1 / (1 + sum))
        ...
        try:
            query = """
                SELECT (COUNT(*) AS ?length) {
                     <%(start)s> dbo:wikiPageWikiLink{%(length)s}|dcterms:subject{%(length)s}|skos:broader{%(length)s} <%(end)s> .
                }
            """ % {
                "start": source["uri"],
                "end": target["uri"],
    ...
    def _count_property_occurence(self, property):
        ...
        try:
            condition = (
                re.sub(r"[^a-zA-Z0-9]+", " ", property["name"])
                .strip()
                .replace(" ", "_")
            )
            query = (
    ...
    def _get_wikipedia_abstract(self, node):
        ...
        try:
            ...
            logger.warning(
                "Failled to retrieve abstract of page %s - %s" % (node["uri"], e)
            )
            node["abstract"] = ""
    ...
    def _get_wikipedia_page(self, node):
        ...
        try:
            if node["type"] != "category":
                ...
                self.sparql.setReturnFormat(JSON)
                results = self.sparql.query().convert()
                wikipedia = results["results"]["bindings"][0]["wikipedia"]["value"]
            ...
            logger.warning(
                "Failled to retrieve wikipedia page for %s - %s" % (node["uri"], e)
            )
            node["wikipedia"] = ""
    ...
    def _get_concept_pairs(self, nodes):
        ...
        return [
            ...
            if a != b and not self._exists_rel(a, b) and not self._exists_rel(b, a)
        ]
