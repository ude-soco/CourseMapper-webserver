...
/* 
generatePopupBarChart(keyphrase: string) {
  ...
  const rawScores = this.getSimilarityScoresAlignedToFixedYaxisPopUp(keyphrase);
...
generatePopupBarChart(keyphrase: string) {
  ...
  // 1. Get similarity scores for clicked keyphrase
  const index = this.keyphrasesImportanceTuple.findIndex(tuple => tuple[0] === keyphrase);
  if (index === -1) return;
  ...
  // 2. Take max 3 DNUs
  const labels = this.notUnderstoodConceptsNames.slice(0, 3);
  const scaledData = labels.map(dnu => (similarityObject[dnu] ?? 0) * 100);
  const dynamicBarColors = labels.map((dnu, i) => this.dnuColors[i] ?? 'red');

  // 3. Destroy old popup chart
  if (this.popupChart) {
    this.popupChart.destroy();
  }

  // 4. Create new popup chart
...
openKeyphrasePopover(popover: OverlayPanel, event: MouseEvent, part: { text: string }) {
  ...
  if (this.lastTarget === target) {
    ...
    setTimeout(() => {
      ...
      this.generatePopupBarChart(this.selectedKeyphrase);
    }, 50);
  }
}
...
showKeyphrases(): boolean {
  return !!this.article?.keyphrases &&
         (this.resourcesPagination?.recommendation_type === '1' ||
          this.resourcesPagination?.recommendation_type === '3');
}
    toggleWhy() {
...
/* 
generatePopupBarChart(keyphrase: string) {
  if (!this.popupBarChartCanvas) return;

  const canvas = this.popupBarChartCanvas.nativeElement;
  const { labels, data, colors } = this.getTop3SimilarityScores(keyphrase);

  // Destroy old chart
  if (this.popupChart) this.popupChart.destroy();
  ...
  this.popupChart = new Chart(canvas, {
    ...
    options: {
      ...
      animation: { duration: 0 },
      scales: {
        x: { beginAtZero: true, min: 0, max: 100 },
        y: {}
      },
