...
openKeyphrasePopover(popover: OverlayPanel, event: MouseEvent, part: { text: string }) {
  ...
  if (this.lastTarget === target) {
    // Same element → just toggle
    popover.toggle(event);
  } else {
    // Different element → update content
    ...
    // Listen once for the hide event, then re-show on the new target
    const sub = popover.onHide.subscribe(() => {
      popover.show(event);
      this.lastTarget = target;
      sub.unsubscribe(); // prevent memory leaks
    });

    popover.hide(); // triggers onHide
  }
}
