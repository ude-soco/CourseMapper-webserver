...
normalizeText(text: string): string {
    return text
      ...
      .replace(/ ?- ?/g, "-")                   // normalize spaced dashes
      .replace(/−/g, "-")                       // replace Unicode minus with ASCII
      .trim();
  }
...
generateParts(
  ...
  // Clean LaTeX-style formatting and normalize the abstract text
  text = this.normalizeText(this.cleanTextForLatex(text));
  console.log("🔹 Normalized abstract:", text);
  ...
  // Normalize and clean keyphrases
  const normalizedKeyphrases: { text: string, dnu: string }[] = keyphrases.map((kp, i) => {
    const raw = Array.isArray(kp) ? kp[0] : kp;
    const cleaned = this.normalizeText(this.cleanKeyphrase(raw));
    console.log(`🔹 Keyphrase[${i}] raw: "${raw}", cleaned: "${cleaned}"`);
    return {
      text: cleaned,
  ...
  normalizedKeyphrases.forEach(({ text: kp, dnu }, idx) => {
    if (!kp) return;
    ...
    while ((match = wordBoundaryRegex.exec(text)) !== null) {
      ...
      console.log(`🔹 Match for keyphrase[${idx}] "${kp}" at index ${match.index}`);
    }
  });

  console.log("🔹 All matches found:", matches);

  // Keep all matches, even overlapping ones (for full coverage)
  const filteredMatches = matches;
  console.log("🔹 Filtered matches (overlaps allowed):", filteredMatches);
  ...
  // Log final abstract parts
  this.abstractParts.forEach((part, i) => {
    console.log(`🔹 Part[${i}] text: "${part.text}", isKeyphrase: ${part.isKeyphrase}`);
  });
}
