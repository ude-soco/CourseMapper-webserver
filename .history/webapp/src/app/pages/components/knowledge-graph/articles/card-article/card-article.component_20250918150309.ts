...
cleanKeyphrase(kp: string): string {
  return kp
    ...
    .replace(/\s+/g, '[-\\s]?')            // collapse multiple spaces/tabs/newlines into one space plus allow optional space or hyphen
...
generateParts(text: string, keyphrases: string[], keyphrases_dnu_similarity_score: any[]) {
  ...
  normalizedKeyphrases.forEach(({ text: kp, dnu }) => {
    ...
    const wordBoundaryRegex = new RegExp(this.escapeRegex(kp), 'gi');
