...
generateParts(
  ...
  // --- Helper: Normalize text and remove invisible characters ---
  ...
  // Clean and normalize the abstract text
  text = normalizeText(this.cleanTextForLatex(text));
  ...
  const normalizedKeyphrases: { text: string; dnu: string }[] = keyphrases.map((kp, i) => {
    ...
    return {
      text: normalizeText(this.cleanKeyphrase(raw)),
  ...
  normalizedKeyphrases.forEach(({ text: kp, dnu }) => {
    ...
    const isSingleWord = !/\s/.test(kp);
    const regex = isSingleWord
      ? new RegExp(`\\b${this.escapeRegex(kp)}\\b`, 'gi')
      : new RegExp(this.escapeRegex(kp), 'gi');
  ...
  // --- Filter overlapping matches but allow nested keyphrases ---
  ...
  for (const match of matches) {
    if (match.index >= lastEnd) {
      ...
      // Nested match inside previous one: still include
      filteredMatches.push(match);
    }
  }

  // --- Build abstract parts ---
  let currentIndex = 0;
  ...
  for (const match of filteredMatches) {
    ...
    currentIndex = Math.max(currentIndex, match.index + match.length);
  }
  ...
  console.log('➡️ Generated abstractParts:', this.abstractParts);
}


/* GOOD generateParts(text: string, keyphrases: string[], keyphrases_dnu_similarity_score: any[]) {
  this.abstractParts = [];
