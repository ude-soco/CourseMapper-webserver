...
export class CardArticleComponent implements AfterViewInit, OnDestroy {
...
@ViewChild('kpPopover') kpPopover!: OverlayPanel;
selectedKeyphrase: string | null = null;
 // state + subscriptions
  private overlayIsVisible = false;
  private onShowSub?: Subscription;
  private onHideSub?: Subscription;
  private reopenSub?: Subscription; // temp subscription used when swapping targets
...
/* 
  ...
  ngAfterViewInit(){
    console.log("coloredBandData:", this.coloredBandData);
     // Keep overlayIsVisible accurate by subscribing to panel events
    if (this.kpPopover) {
      this.onShowSub = this.kpPopover.onShow?.subscribe(() => {
        this.overlayIsVisible = true;
      });
      this.onHideSub = this.kpPopover.onHide?.subscribe(() => {
...
openKeyphrasePopover(popover: OverlayPanel, event: MouseEvent, part: { text: string }) {
    const target = (event.currentTarget || event.target) as any;

    // set content
    this.selectedKeyphrase = part.text;

    // optional: notify parent analytics
    ...
    // ensure template reflects selectedKeyphrase
    this.cdr.detectChanges();

    // If overlay is not visible, simply show it anchored to the clicked span
    ...
    // If clicked the same element -> toggle (close if open)
    if (this.lastTarget === target) {
      popover.toggle(event);
      // lastTarget unchanged (toggle behavior preserved)
      return;
    }

    // Different target: hide current, then re-show when hide finishes
    // Clean up any previous reopen subscription
    this.reopenSub?.unsubscribe();
    this.reopenSub = popover.onHide?.subscribe(() => {
      // Once hidden, show anchored to the new target
      popover.show(event, target);
      this.lastTarget = target;
      this.reopenSub?.unsubscribe();
      this.reopenSub = undefined;
    });

    // Trigger hide (onHide will fire when finished)
    popover.hide();

    // Fallback: if onHide doesn't fire for some reason, force show after a short delay
    setTimeout(() => {
      if (this.reopenSub) { // still waiting for onHide
        this.reopenSub.unsubscribe();
        this.reopenSub = undefined;
        popover.show(event, target);
        this.lastTarget = target;
      }
    }, 200); // 200ms fallback (adjust if needed)
  }


emitKeyphraseAnalytics() {
  if (this.selectedKeyphrase) {
    // do whatever you need here (service call, etc.)
  }
}
/*  These three showPopup(text: string, clientX: number, clientY: number, event: MouseEvent) {
