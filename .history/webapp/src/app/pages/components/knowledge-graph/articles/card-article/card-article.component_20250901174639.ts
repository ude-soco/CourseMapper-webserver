...
openKeyphrasePopover(popover: OverlayPanel, event: MouseEvent, part: { text: string }) {
  ...
  if (this.lastTarget === target) {
    // same element: toggle (close if open)
    popover.toggle(event);
  } else {
    // different element: show / reposition with updated content
    this.selectedKeyphrase = part.text;
    this.keyphraseClicked.emit({ keyphrase: part.text, clientX: event.clientX, clientY: event.clientY });
