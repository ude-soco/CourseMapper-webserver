...
getTop3SimilarityScores(keyphrase: string): { labels: string[], data: number[], colors: string[] } {
  const index = this.keyphrasesImportanceTuple.findIndex(tuple => tuple[0] === keyphrase);
  if (index === -1) return { labels: [], data: [], colors: [] };

  const similarityObj = this.keyphrases_dnu_similarity_score[index];
  const labels = this.notUnderstoodConceptsNames.slice(0, 3); // max 3
  const data = labels.map(label => (similarityObj[label] ?? 0) * 100);
  const colors = labels.map((label, i) => this.dnuColors[i] ?? 'red');
...
openKeyphrasePopover(popover: OverlayPanel, event: MouseEvent, part: { text: string }) {
  ...
  if (this.lastTarget === target) {
    ...
    setTimeout(() => {
      ...
      this.generatePopupBarChart(part.text);
    }, 50);
  }
}
