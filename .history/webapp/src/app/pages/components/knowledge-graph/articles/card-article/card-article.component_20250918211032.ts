...
generateParts(
  ...
  // Build expanded keyphrases with variants
  ...
  keyphrases.forEach((kp, i) => {
    ...
    variants.forEach(v => {
      expandedKeyphrases.push({
        text: v,
        dnu,
        original: cleaned, // always keep reference to original form
      });
    });
  });
  ...
  expandedKeyphrases.forEach(({ text: kp, dnu, original }) => {
    ...
    while ((match = regex.exec(cleanedAbstract)) !== null) {
      matches.push({
        ...
        kp: original, // link back to original keyphrase
        dnu,
      });
    }
  });
  ...
  for (const match of filteredMatches) {
    ...
    this.abstractParts.push({
      ...
      keyphraseMeta: {
        concept: match.dnu,
        originalKeyphrase: match.kp, // ✅ reference original form
