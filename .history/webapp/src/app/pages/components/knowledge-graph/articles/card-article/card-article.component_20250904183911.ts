...
openKeyphrasePopover(popover: OverlayPanel, event: MouseEvent, part: { text: string }) {
  ...
  // Show overlay at custom position (optional offset)
  const xOffset = 0;   // pixels from the left
  const yOffset = 20;  // pixels below the target
  popover.show(event, null); // null = use event target as default
  const panel = (popover as any).containerViewChild?.nativeElement;
  if (panel) {
    panel.style.top = (panel.offsetTop + yOffset) + 'px';
    panel.style.left = (panel.offsetLeft + xOffset) + 'px';
  }
}
...
/* showKeyphrases(): boolean {
  console.log(this.resourcesPagination?.recommendation_type ,!!this.article?.keyphrases 
