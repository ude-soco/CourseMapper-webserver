...
generateParts(text: string, keyphrases: string[], keyphrases_dnu_similarity_score: any[]) {
  ...
  const normalizedKeyphrases: { regex: RegExp, dnu: string, raw: string }[] = keyphrases.map((kp, i) => {
    const raw = Array.isArray(kp) ? kp[0] : kp;
    const regex = this.makeFlexibleRegex(raw);
    console.log(`➡️ Keyphrase[${i}] raw: "${raw}", regex: ${regex}`);
    return {
      regex,
      raw,
  ...
  normalizedKeyphrases.forEach(({ regex, raw, dnu }) => {
    let foundAny = false;
  ...
  // Sort matches by position then by length
