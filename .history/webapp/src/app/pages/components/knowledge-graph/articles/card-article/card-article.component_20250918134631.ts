...
/* 
  ...
  const normalizedKeyphrases: { text: string, dnu: string }[] = keyphrases.map((kp, i) => {
    ...
    console.log(`➡️ Keyphrase[${i}] raw: "${raw}", cleaned: "${cleaned}"`);
    return {
      text: cleaned,
      dnu: Object.keys(keyphrases_dnu_similarity_score[i])[0]
    };
  ...
  normalizedKeyphrases.forEach(({ text: kp, dnu }) => {
    ...
    let foundAny = false;
    let match: RegExpExecArray | null;
    while ((match = wordBoundaryRegex.exec(cleanedAbstract)) !== null) {
      matches.push({
        index: match.index,
        length: match[0].length,
        kp,
    ...
    if (!foundAny) {
      console.warn(`⚠️ No match in abstract for keyphrase: "${kp}"`);
    }
  });
 ...
 const filteredMatches: typeof matches = [];
  ...
  generateParts(text: string, keyphrases: string[], keyphrases_dnu_similarity_score: any[]) {
  text = this.cleanTextForLatex(text); // clean abstract
  ...
  // Step 1: Find all matches (can overlap)
  const matches: { start: number; end: number; kp: string; dnu: string }[] = [];
  normalizedKeyphrases.forEach(({ text: kp, dnu }) => {
    if (!kp) return;
    const regex = new RegExp(`\\b${this.escapeRegex(kp)}\\b`, "gi");
    let match: RegExpExecArray | null;
    while ((match = regex.exec(text)) !== null) {
      matches.push({ start: match.index, end: match.index + match[0].length, kp, dnu });
    }
  });

  // Step 2: Sort matches by start position, then by length descending
  matches.sort((a, b) => a.start - b.start || b.end - b.start - (a.end - a.start));

  // Step 3: Create a boolean array to track highlighted characters
  const highlightMap = Array(text.length).fill(null as null | { kp: string; dnu: string });

  for (const m of matches) {
    for (let i = m.start; i < m.end; i++) {
      highlightMap[i] = { kp: m.kp, dnu: m.dnu }; // overwrite if needed, last wins
    }
  }

  // Step 4: Build abstractParts sequentially
  this.abstractParts = [];
  let buffer = "";
  let currentKeyphrase: null | { kp: string; dnu: string } = null;

  for (let i = 0; i < text.length; i++) {
    const mark = highlightMap[i];
    if (!currentKeyphrase && !mark) {
      buffer += text[i]; // plain text
    } else if (!currentKeyphrase && mark) {
      if (buffer) this.abstractParts.push({ text: buffer, isKeyphrase: false });
      buffer = text[i];
      currentKeyphrase = mark;
    } else if (currentKeyphrase && mark && currentKeyphrase.kp === mark.kp) {
      buffer += text[i]; // continue same keyphrase
    } else if (currentKeyphrase && (!mark || currentKeyphrase.kp !== mark.kp)) {
      // flush previous keyphrase
      this.abstractParts.push({
        text: buffer,
        isKeyphrase: true,
        keyphraseMeta: {
          concept: currentKeyphrase.dnu,
          color: this.getColorForConcept(currentKeyphrase.dnu)
        }
      });
      buffer = mark ? text[i] : text[i];
      currentKeyphrase = mark;
    }
  }
  ...
  if (buffer) {
    if (currentKeyphrase) {
      ...
      this.abstractParts.push({ text: buffer, isKeyphrase: false });
    }
  }
}
