...
escapeRegexForKeyphrase(kp: string): string {
  // Escape all regex chars except [-\s]+
  return kp.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
}
...
cleanKeyphrase(kp: string): string {
  return kp
    ...
    .toLowerCase()
    .trim()
    // replace spaces and hyphens with flexible pattern AFTER trimming
    .replace(/[\s-]+/g, '[-\\s]+');
}
...
generateParts(text: string, keyphrases: string[], keyphrases_dnu_similarity_score: any[]) {
  ...
  normalizedKeyphrases.forEach(({ text: kp, dnu }) => {
    ...
    const safeKp = this.escapeRegexForKeyphrase(kp); // escape other regex chars
const flexibleRegex = new RegExp(safeKp, 'gi');  // use this regex


    let foundAny = false;
    let match: RegExpExecArray | null;
    while ((match = flexibleRegex.exec(cleanedAbstract)) !== null) {
      matches.push({
        index: match.index,
