...
cleanKeyphrase(kp: string): string {
  return kp
    ...
    .replace(/[^\w\s]/g, '')    // strip punctuation (optional, may be too aggressive)
    .trim();
}
...
getSimilarityScoresAlignedToFixedYaxisPopUp(keyphrase: string): number[] {
  // robust find: compare cleaned keyphrases
  const cleanedKey = this.cleanKeyphrase(keyphrase);
  console.log('Clicked keyphrase (cleaned):', cleanedKey);
console.log(
  'Available article.keyphrases (cleaned):',
  this.article.keyphrases.map(tuple => {
    const candidate = Array.isArray(tuple) ? String(tuple[0]) : String(tuple);
    return this.cleanKeyphrase(candidate);
  })
);
  ...
  // Map scores to fixed Y-axis order
  return this.conceptsNames.map(dnu => {
