  ...
  ngOnInit() {
    ...
    this.coloredBandData = {
      document_concept_similarity_colorband: this.concepts.reduce((acc, concept, i) => {
        acc[concept.name] = this.article.document_concept_similarity[concept.name] || 0;
        return acc;
      }, {}),
    ...
    this.generateParts(
      ...
      this.article.keyphrases_concept_similarity_score
    );
...
generateParts(text: string, keyphrases: string[], keyphrases_concept_similarity_score: any[]) {
  this.abstractParts = [];
  ...
  const normalizedKeyphrases: { text: string, dnu: string }[] = keyphrases.map((kp, i) => {
    ...
    return {
      ...
      dnu: Object.keys(keyphrases_concept_similarity_score[i])[0]
    };
  });
...
truncateParts(parts: { text: string, isKeyphrase: boolean, keyphraseMeta?: any }[], maxLength: number) {
  ...
  const similarityObject = this.article.keyphrases_concept_similarity_score[index];
console.log('Similarity object:',this.article.keyphrases_concept_similarity_score[index]);
