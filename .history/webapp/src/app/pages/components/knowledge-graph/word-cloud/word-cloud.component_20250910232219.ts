...
quantizeWeight(weight: number, numCategories: number = 20, minWeight = 0, maxWeight = 1): number {
  const step = (maxWeight - minWeight) / numCategories;
  const category = Math.ceil((weight - minWeight) / step); // 1..numCategories
  return Math.min(category, numCategories); // cap at max
}
...
const quantizedWeight = this.quantizeWeight(weight, numCategories);
return [cleaned, quantizedWeight];
});
...
gridSize: 10,
weightFactor: (size) => size * 5, // size=1..20, scales fonts to 5px..100px
