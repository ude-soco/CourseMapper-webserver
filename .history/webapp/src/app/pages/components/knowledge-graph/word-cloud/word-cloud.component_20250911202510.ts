...
canvas.style.marginBottom = '0';
  ...
  // --- NEW: compute min and max weights per article ---
const weights = this.keyphrasesImportanceTuple.map(([_, w]) => w);
const minWeight = Math.min(...weights);
const maxWeight = Math.max(...weights);


function scaleWeight(w: number, minWeight: number, maxWeight: number, minFont = 10, maxFont = 50): number {
  if (maxWeight === minWeight) return (minFont + maxFont) / 2; // handle all weights equal
...
return [cleaned, scaleWeight(weight, minWeight, maxWeight)]; // scaled font size
});
...
gridSize: 10,
weightFactor: (size) => size , // size=1..20, scales fonts to 5px..100px
...
getSimilarityScoresAlignedToFixedYaxis(keyphrase: string): number[] {
  ...
  const index = this.keyphrasesImportanceTuple.findIndex(tuple => {
    ...
    return this.cleanKeyphrase(rawPhrase) === cleanedKey;
  });
  ...
  // Map scores based on fixed Y-axis order
  return this.notUnderstoodConceptsNames.map(dnu => {
    return similarityObject && Object.prototype.hasOwnProperty.call(similarityObject, dnu)
