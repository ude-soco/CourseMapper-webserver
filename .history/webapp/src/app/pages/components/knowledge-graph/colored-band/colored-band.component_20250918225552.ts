...
ngOnInit(): void {
  ...
  if (tooltipParts.length > 0) {
    const contentLabel = this.contentType === 'video' ? 'This video' : 'This article';
    this.tooltipText = `${contentLabel} "${this.resourceTitle}" is ${tooltipParts.join(' and ')}.`;
  }
}
...
/* ngOnInit(): void {
  ...
  for (const [dnu, sim] of entries) {
    ...
    if (tag) {
      ...
      tooltipParts.push(`"${dnu}": ${similarityPercent}%`);
    }
  }

  // Build a single tooltip for all concepts
  if (tooltipParts.length > 0) {
    this.tooltipText = `This article "${this.resourceTitle}" has similarity scores with concepts: ${tooltipParts.join(', ')}.`;
  }
} */
