...
class Recommender:
    ...
    def compute_concept_keyphrase_similarity(self,row,concept_nodes):
    ...
    def compute_concept_document_similarity(self,row,concept_nodes):

        for concept in concepts_nodes:
                oncept_embedding = [float(x.strip()) for x in concept["final_embedding"].split(',')]
    ...
    def post_retrieve_keyphrases(self,data,dnu_concepts,mid):
        ...
        for index, row in data.iterrows():
            ...
            if text:
                ...
                """ for i,keyphrase_embedding in enumerate(row["individual_keyphrase_embeddings"]):
                    ...
                    sorted_similarity= self.compute_concept_keyphrase_similarity(row,concept_nodes)
                    keyphrases_dnu_similarities.append(sorted_similarity)
                    ...
                    """ for concept in concepts_nodes:
                        ...
                        document_similarity[concept["name"]]=compute_cosine_similarity_with_embeddings(concept_embedding
                                                                                                    ,row["document_embedding"]) """
                document_similarity=self.compute_concept_document_similarity(row,concept_nodes)                                                                                    
        ...
        def process_keyphrases_infos(row):
            kp_infos = row["keyphrases_infos"]
            emb_count = len(row["individual_keyphrase_embeddings"])
            kp_infos = json.loads(row["keyphrases_infos"])
            
            # Convert each keyphrase info list to a tuple and truncate
            return [tuple(kp) for kp in kp_infos[:emb_count]]
        

        data["keyphrases"] = data.apply(process_keyphrases_infos, axis=1)
