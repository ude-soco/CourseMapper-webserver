    ...
    def post_retrieve_keyphrases(self,data,dnu_concepts,mid):
        ...
        for index, row in data.iterrows():
            ...
            if text:
                ...
                document_similarity={}
                for i, kp in enumerate(row["keyphrases_infos"]):
                    #keyphrases.append(kp)
                    #keyphrase_embedding=self.embedding.encode(kp[0])
                    print("individual keyphrase embeddings: ", row["individual_keyphrase_embeddings"])
                    keyphrase_embedding=row["individual_keyphrase_embeddings"][i]
