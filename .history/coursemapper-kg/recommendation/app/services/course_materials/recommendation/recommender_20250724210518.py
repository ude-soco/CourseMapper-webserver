...
""" def get_tensor_from_embedding(embedding):
    ...
    if isinstance(embedding, list):
        # Convert list of strings/floats to NumPy float array
        np_array = np.array(embedding, dtype=np.float32)
    else:
        # Extract float strings from messy input
        float_strings = re.findall(r"-?\d+(?:\.\d+)?(?:e[+-]?\d+)?", embedding.strip())
        print(float_strings)
        float_values = [float(x) for x in float_strings]
        np_array = np.array(float_strings, dtype=np.float32)
    ...
    def post_retrieve_keyphrases(self,data,dnu_concepts):
        ...
        for index, row in data.iterrows():
            ...
            if text:
                ...
                for dnu in dnu_concepts:
                    document_similarity[dnu["name"]]=self.compute_cosine_similarity_with_text(dnu["name"],text)
        ...
        print(data.iloc[0])
        print("Concept list: ", dnu_concepts)
